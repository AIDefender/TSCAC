...
if __name__ == "__main__":
    ...
    with open(get_path() + "/config/ddpg.yaml","r") as f:
        config=yaml.safe_load(f)
    args=SN(**config)

    policy=DDPG.DDPG(args)
    model_list = os.listdir(get_path() + "/results/ddpg/")
    model_list.remove('_sources')
    model_list = sorted(model_list)
    policy.load(get_path() + "/results/ddpg/49/models/") #load trained DDPG policy
